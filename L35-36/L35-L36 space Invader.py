...
for i in range(num_of_enemies):
    enemyImg.append(pygame.image.load('L35-36/enemy.png'))
...
def game_over_text():
    over text = over_font.render("GAME OVER", True, (255, 255, 255))
...
def isCollision(enemyX, enemyy, bulletX, bullety):
    distance = math.sqrt(math.pow(enemy bulletx, 2) + (math.pow(enemyy - bullety, 2)))
    if distance < 27:
        return True
    else:
        return False
...
while running:
            ...
            if event.type == pygame.KEYDOWN:
                ...
                if event.key == pygame.K_SPACE:
                    if bullet_state == "ready":
                        bulletSound = mixer.Sound("L35-36/laser.wav")
                        bulletSound.play()
    ...
    for i in range(num_of_enemies):
        ...
        if collision:
            explosionSound = mixer. Sound("L35-36/explosion.wav")
            explosionSound.play()
            bullety = 380
            bullet_state = "ready"
            score_value += 1
